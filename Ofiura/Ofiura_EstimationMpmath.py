...
def grandCountGN_UltraX1_mpmath_mantissa (funcf, jacf,  measdata:list, binit:list, c, NSIG=3, implicit=False, verbose=False, mantissa=None):
    """
    Производит оценку коэффициентов по методу Гаусса-Ньютона с переменным шагом
    В стандартный поток вывода выводит отладочную информацию по каждой итерации
    :param funcf callable функция, параметры по формату x,b,c, на выходе вектор (pure Python)
    ...
    while (condition):
        if mantissa:
            if (numiter>len(mantissa)-1):
                wdps=mantissa[-1]
            else:
                wdps=mantissa[numiter]
        else:
            wdps=15

        with mpm.workdps(wdps):
            ...
            if verbose:
                print ("Sk:",Sk)
                print ("Iteration {0} mu={1} delta={2} deltamu={3} resb={4} mantissa={5}".format(numiter, mu, deltab, deltab*mu, b, wdps))
